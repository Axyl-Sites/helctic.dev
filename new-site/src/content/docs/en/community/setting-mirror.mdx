...
At the beginning, Arch-based Helctic synchronized mirrors by git protocol but it was not suitable for package mirroring for the nature of git usage. One suitable option to consider has been rsync protocol.
...
The purpose of this guide is to set a mirror for Helctic ISO images and Helctic Arch packages. In Helctic Nix there is no need to mirror packages because package mirror system is not used on Nix environment.
...
* CPU: >= 4 cores
* RAM: >= 8 GB
* Storage: the Helctic repository size is around 5 GB, so you can set at least x3 repository size as server storage
* Bandwidth: >= 1 Gbps
...
```shell
sudo mkdir -p /srv/mirrors/Helctic{,-images}
sudo chown -R rsyncuser:rsyncuser /srv/mirrors/Helctic
sudo chown -R rsyncuser:rsyncuser /srv/mirrors/Helctic-images
```
...
[Helctic]
path = /srv/mirrors/Helctic
comment = The Helctic Hub
read only = true

[Helctic-images]
path = /srv/mirrors/Helctic-images
comment = The Helctic ISO images
read only = true
```
...
{/*
Download the public key of `hub.Helcticos.org` and set it among your authorized keys by:
```shell
su - rsyncuser
mkdir -p ~/.ssh/
chmod 0700 ~/.ssh/
wget -O - -q https://hub.Helcticos.org/mirrorkey.pub >> ~/.ssh/authorized_keys
...
```
hub.Helcticos.org will push and trigger delta changes to the new destination mirror server.
NOTE: currently we use PULL approach instead of PUSH approach, so it is not the main Helctic mirror server that pushes changes to your new mirror, but it is the new mirror that pulls changes from main Helctic mirror server.
*/}

{/*
To pull and synchronize Helctic packages, you must create a SSH keypair and provide the public key to Helctic Team. In particular, generate a SSH keypair:
```shell
su - rsyncuser
...
To keep your mirror up-to-date, you should regularly sync with the Helctic repositories. First, enable `cron` service:
...
```
0 *\/1 * * * rsync -avzzlr --delete -e ssh rsyncuser@hub.Helcticos.org:/srv/mirrors/ /srv/mirrors/
```
*/}
```
0 */1 * * * rsync -avzzlr --delete rsync://hub.Helcticos.org/Helctic /srv/mirrors/Helctic
0 */1 * * * rsync -avzzlr --delete rsync://hub.Helcticos.org/Helctic-images /srv/mirrors/Helctic-images
```
...
Create `/etc/nginx/sites-available/Helcticmirror` with the following content:
```
server {
    listen 80;
...
```shell
sudo ln -s /etc/nginx/sites-available/Helcticmirror /etc/nginx/sites-enabled/
```
...
Finally, on your Helctic or Arch Linux environment, you can test it on your test Helctic client by adding your mirror in `/etc/pacman.d/Helctic-mirrorlist` by:
```
...
After obtaining the certificate, Certbot will automatically update your Nginx configuration to use HTTPS. If, for any reason, this doesn't happen, you can manually edit `/etc/nginx/sites-available/Helcticmirror` by:
```
server {
    server_name your_mirror_domain.com;
...
| -------- | ---- | ----------- | ---------- |
| Rsync | 873/TCP | hub.Helcticos.org | 89.116.236.99 |

### Publish

{/*
At this point the new mirror server cannot still retrieve information from the main Helctic mirror. To do that, get the content of the previously created SSH public key by:
```shell
su -s rsyncuser
cat ~/.ssh/id_rsa.pub
```
*/}
Finally, to be deployed to all users, your mirror must be added communicated to Helctic Team in order to be added in the official Helctic mirrorlist.

Send the following information to **keeper@Helcticos.org**:
* public hostname
* scheduled sync time
* email to inform you in case we make changes on mirroring design

Once Helctic Team accepts your request, you will be informed your mirror has been added on Helctic mirrorlist.
