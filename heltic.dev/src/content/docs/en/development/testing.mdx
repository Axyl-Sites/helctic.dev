...
Purpose of this chapter is to test a GitHub workflow of Helctic repository in a local machine.
...
Clone Helctic repository:
```shell
git clone https://github.com/Helctic-OS/Helctic
cd Helctic
```
...
Boot into Helctic Live Environment by using Helctic ISO.
...
nix-channel --update
```
Then, generate the hardware configuration and get the Helctic Nix configuration:
```shell
mkdir -p /mnt/etc/nixos
nix-shell -p nixos-install-tools --command "nixos-generate-config --root /mnt"
curl -o /tmp/Helctic-nix.zip https://codeload.github.com/Helctic-OS/Helctic-nix/zip/refs/heads/main
unzip /tmp/Helctic-nix.zip -d /tmp/
cp -rf /tmp/Helctic-nix-main/nixos/home-manager /tmp/Helctic-nix-main/nixos/hosts /tmp/Helctic-nix-main/nixos/modules /tmp/Helctic-nix-main/nixos/pkgs /tmp/Helctic-nix-main/nixos/users /tmp/Helctic-nix-main/nixos/configuration.nix /tmp/Helctic-nix-main/nixos/default.nix /mnt/etc/nixos/
...
```
Finally, run the Helctic Nix installation:
```shell
