...
If you wish to submit a new pentesting tool that is not in any Arch Linux or Helctic repository, this section will help you.

Pentesting tools are mainly maintained in Helctic repository, so your new pentesting tools must be stored there.
...
1. Create a **PKGBUILD** file containing the rules for compiling and installing `mytoolname`. According to the type of the tool, it can be useful to start by one of [BlackArch PKGBUILD templates](https://github.com/BlackArch/blackarch-pkgbuilds)
   ...
   * copy the content of the PKGBUILD template file inside a new file named `PKGBUILD` created in `Helctic/packages/pentesting/<mytoolname>` folder.
2. Fill the variable inside PKGBUILD according to the tool information. If you are new on PKGBUILD, give a look to the [official Arch Linux documentation](https://wiki.archlinux.org/title/PKGBUILD). You can check several examples in [Helctic repository](https://github.com/Helctic-OS/Helctic/tree/main/packages/pentesting)
   * when you write information inside PKGBUILD, there are several best practices you must follow:
      ...
      * In `groups=`, insert the right Helctic categories
...
7. Go to your draft pull request and submit it. You need to wait for Helctic maintainers that will check your submission and merge your pull request.
...
:::caution
If your submitted tool has some dependencies that are not in Helctic or Arch Linux repositories, you need to create a PKGBUILD dedicated to it in Helctic repository itself. For example, in your forked repository you should create the PKGBUILD file in another directory as `Helctic/packages/pentesting/<yourdependency>/PKGBUILD`. The PKGBUILD related to a depedency of a tool does not need of the field `groups=('Helctic' MORE_Helctic_GROUPS_HERE).`
:::

## Updating a package

If you noticed that the repository is not keeping the latest version of a tool:
1. Access to its PKGBUILD in [Helctic Repository](https://github.com/Helctic-OS/Helctic/tree/main/packages/pentesting) by visiting its directory by **https://github.com/Helctic-OS/Helctic/tree/main/packages/pentesting/toolname** and inside it there should be a PKGBUILD file.
