---
title: Hyprland
description: Hyprland environment in Helctic.
---
...
In Helctic, Hyprland has been been set up to make the user experience smoother in a window manager environment.
...
Helctic's Hyprland config is based on the wonderful [dotfiles from @end-4](https://github.com/end-4/dots-hyprland).
...
To install Helctic Hyprland environment on side of an existing environment, run:
```shell
sudo pacman -S Helctic-hyprland-config
dconf load /org/gnome/desktop/ < /usr/share/Helctic-hyprland-config/dconf-desktop.ini
```
Then reboot. On Login Manager, select **Hyprland** session and login.

If your theme is not correctly displayed, run `Helctic-theme-tweak` to set or change the current environment theme.
